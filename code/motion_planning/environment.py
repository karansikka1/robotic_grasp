...
class UltraTask(ManipulationEnv):
    ...
    def _check_success(self):
        """
        Check for a red -> green -> blue stack.

        Returns:
            bool: True when red supports green and green supports blue.
        """
        red_height = self.sim.data.body_xpos[self.cubeA_body_id][2]
        green_height = self.sim.data.body_xpos[self.cubeB_body_id][2]
        blue_height = self.sim.data.body_xpos[self.cubeC_body_id][2]

        return bool(
            red_height < green_height < blue_height
            and self.check_contact(self.cubeA, self.cubeB)
            and self.check_contact(self.cubeB, self.cubeC)
        )
