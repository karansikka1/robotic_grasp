...
from .teacher import PrivilegedStackTeacher, Stage, TeacherDecision, TeacherFailure
...
KINDS = ('green_drop', 'green_misalignment', 'blue_misalignment')

class FailureDetector:
    """Causal triggers for an unsupported fall or sustained unsuccessful closure."""
    def __init__(self, initial):
        self.rest = initial.positions[:, 2].copy()
        self.previous = initial
        self.green_held_streak = 0
        self.lifted_green = False
        self.loss = None
        self.close_streak = {1: 0, 2: 0}

    def update(self, state, action, step):
        height = state.positions[1, 2] - self.rest[1]
        self.green_held_streak = self.green_held_streak + 1 if state.grasped[1] and height >= .05 else 0
        self.lifted_green |= self.green_held_streak >= 5
        if self.previous.grasped[1] and not state.grasped[1] and self.lifted_green:
            self.loss = {'step': step, 'z': float(self.previous.positions[1, 2]),
                         'release_command': 'open' if action[6] < 0 else 'close'}
        event = None
        if self.loss is not None:
            fall = self.loss['z'] - state.positions[1, 2]
            if state.grasped[1] or step-self.loss['step'] > 30:
                self.loss = None
            elif fall >= .035 and not state.object_contacts[0]:
                event = {'kind': 'green_drop', 'phase': 0, 'step': step,
                         'grasp_loss_step': self.loss['step'], 'downward_travel_m': float(fall),
                         'release_command': self.loss['release_command']}
        base_intact = bool(state.object_contacts[0] and state.positions[1, 2] > state.positions[0, 2]+.025)
        for index in (1, 2):
            delta = state.eef_position - state.positions[index]
            xy = float(np.linalg.norm(delta[:2]))
            relevant = (not base_intact) if index == 1 else base_intact
            near_closed = (relevant and action[6] > 0 and not state.grasped.any()
                           and abs(delta[2]) < .035 and .005 <= xy < .08)
            self.close_streak[index] = self.close_streak[index]+1 if near_closed else 0
            if event is None and self.close_streak[index] >= 20:
                event = {'kind': 'green_misalignment' if index == 1 else 'blue_misalignment',
                         'phase': index-1, 'step': step, 'closed_without_grasp_steps': self.close_streak[index],
                         'grip_minus_object_m': delta.tolist(), 'xy_offset_m': xy}
        self.previous = state
        return event

class PolicyRecoveryTeacher(RecoveryTeacher):
    """Recovery without injected disturbances; initialize at a learner failure."""
    def __init__(self, initial, state, step, event, seed):
        super().__init__(DisturbanceConfig(category='clean'), seed, initial)
        self.teacher.phase = event['phase']
        self._start_recovery(state, step, event['kind'])

    def decide(self, simulator, state, step):
        if self.recovery_start is not None:
            decision = self._recovery_decision(state, step)
            if decision is not None:
                return decision
        self.last_mode = 'normal'
        moving = self.teacher.phase+1
        if self.teacher.stage in TRANSPORT_STAGES and not state.grasped[moving]:
            self._start_recovery(state, step, 'lost grasp during correction')
            return self._recovery_decision(state, step)
        try:
            return self.teacher.decide(simulator)
        except TeacherFailure as error:
            if self.teacher.stage not in (Stage.MOVE_ABOVE, Stage.DESCEND, Stage.CLOSE,
                                          Stage.LIFT, Stage.TRANSFER, Stage.LOWER):
                raise
            self._start_recovery(state, step, str(error))
            return self._recovery_decision(state, step)
