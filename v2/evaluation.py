...
def evaluate_policy(
    policy_fn, evaluation_dir, *, task_config=LiftTaskConfig(),
    simulator_factory=None, observation_adapter=privileged_policy_observation, **kwargs,
):
    ...
    metrics = evaluate_base_policy(
        ...
        simulator_factory=simulator_factory or (lambda: GreenLiftSimulator(task_config)),
    ...
    metrics["summary"].update({
        "contact_rate": float(np.mean([episode["contact_seen"] for episode in episodes])),
        "final_contact_rate": float(np.mean([episode["bilateral_contact"] for episode in episodes])),
        "mean_contact_steps": float(np.mean([episode["contact_steps"] for episode in episodes])),
        "mean_max_contact_streak_steps": float(np.mean([episode["max_contact_streak_steps"] for episode in episodes])),
        "mean_contact_loss_count": float(np.mean([episode["contact_loss_count"] for episode in episodes])),
